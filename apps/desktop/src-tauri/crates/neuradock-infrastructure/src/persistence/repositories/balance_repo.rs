...
impl BalanceRepository for SqliteBalanceRepository {
    ...
    async fn find_by_account_id(
        ...
        let query = "SELECT account_id, current, total_consumed, total_quota, last_checked_at FROM balances WHERE account_id = ?1";
    ...
    async fn find_all(&self) -> Result<Vec<Balance>, DomainError> {
        let query = "SELECT account_id, current, total_consumed, total_quota, last_checked_at FROM balances ORDER BY last_checked_at DESC";
    ...
    async fn find_stale_balances(&self, hours_threshold: i64) -> Result<Vec<Balance>, DomainError> {
        ...
        let query = "SELECT account_id, current, total_consumed, total_quota, last_checked_at FROM balances WHERE last_checked_at < ?1 ORDER BY last_checked_at ASC";
